...
$start = ($page-1)*$num;
   ...
   $sql= "SELECT * FROM `book` LIMIT $start, $num";
...
<body>
        <div class="container">
          ...
          <div class="row justify-content-center">
            ...
            border-radius:8px;
            outline:none;
            padding:8px;"name="search"> 
            
            <button style="margin-top:10px;position:absolute;background:rgb(252, 200, 156);border:none;border-radius:50%;"type="submit" class="text-dark"><i class="material-icons"style="font-size:30px">search</i></button>
            </form>
             </div>
          <br>
          <div class="row">
              ...
              <?php 
                ...
                $p_librarylist = $con->query($p_sql) or die ($con->error);
                $total = mysqli_num_rows($p_librarylist);
                
                $total_rec = ceil($total/$num);
                
                if($page > 1){
                  echo "<a  href='index.php?page=".($page-1)."' class='text-dark'style='font-size:60px'>&laquo;</a>";
                  
                }
                for($i = 1; $i<$total_rec; $i++){
                  echo "<a style='display:none'href='index.php?page=".$i."' class='text-light'>$i</a>";
                }
                
                if($i > $page){
                  echo "<a  href='index.php?page=".($page+1)."'class='text-dark'style='font-size:60px'>&raquo;</a>";
                }
                ?>
          </div>
        </div>
